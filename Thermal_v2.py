...
# Load YOLOv8 Model once at the beginning
model = YOLO("thermal_7.pt")  # Replace with your trained model
...
# Text offset for displaying confidence
text_offset = 10  # Distance to the right of the bounding box

# Threshold for detection confidence
confidence_threshold = 0.8  # Only consider detections with confidence > 0.8
...
while True:
    # Define temperature range for detection (in °C)
    min_temp = 10  # Minimum temperature in °C
    max_temp = 35  # Maximum temperature in °C

    # Capture AMG8833 thermal data (8x8 matrix)
    data = np.array(sensor.pixels)

    # Filter out values outside the defined temperature range
    data[data < min_temp] = min_temp  # Set values below min_temp to min_temp
    data[data > max_temp] = max_temp  # Set values above max_temp to max_temp
    ...
    # Extract boxes, scores, and class ids from YOLO results
    ...
    for i, box in enumerate(final_boxes):
        ...
        # Only count "Leeches" class (class 0)
        if class_ids[i] == 0:  # If the detected class is "Leech"
    ...
    # Create a new image to display the class counts and percentage confidence on the side
    side_panel = np.zeros((large_thermal_image.shape[0], 300, 3), dtype=np.uint8)  # Space for counts and percentages
    ...
    # Display the percentage threshold (confidence) below the count
    y_offset += 40  # Move the text down to leave space for the count
    for i, conf in enumerate(scores):
        if class_ids[i] == 0:  # Only display percentage for "Leech" class
            percentage = f"{conf * 100:.2f}%"  # Format confidence as percentage
            cv2.putText(side_panel, percentage, (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            y_offset += 30  # Space between the percentage values

    # Combine the side panel with the thermal image
    combined_image = np.hstack((large_thermal_image, side_panel))

    # Show YOLOv8 detection output with the class count and percentages on the side
    ...
    time.sleep(0.5)  # Add sleep for smoother display
